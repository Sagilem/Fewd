...
class THtml extends AModule
{
	...
	public function CompileLess(
		string       $lessFilename,
		string       $cssFilename  = '',
		string|null &$errorMessage = '') : string
	{
		...
		if(!is_file($cssFilename) || (filemtime($lessFilename) > filemtime($cssFilename)))
		{
			...
			// Loads it
			if($this->_LessCompiler === null)
			{
				include __DIR__ . '/vendor/wikimedia/less.php/lessc.inc.php';

				$this->_LessCompiler = $this->MakeLessParser();
			}

			// Compiles file
			try
			{
				// Gets domain to use for relative to absolute image links
				$domain = $this->Core()->Protocol() . $this->Core()->Domain();

				$this->_LessCompiler->parseFile($lessFilename, $domain);
				$css = $this->_LessCompiler->getCss();
	...
	//------------------------------------------------------------------------------------------------------------------
	// Internal maker : Less_Parser
	//------------------------------------------------------------------------------------------------------------------
	protected function MakeLessParser() : Less_Parser
	{
