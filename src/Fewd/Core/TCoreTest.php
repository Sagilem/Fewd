...
class TCoreTest extends ATest
{
	...
	protected function RunPaths(TCore $core)
	{
		...
		$this->CheckFalse($core->IsAbsoluteFilename('https://fewd.org'           ));
		$this->CheckFalse($core->IsAbsoluteFilename('ftp://fewd.org'             ));
		$this->CheckFalse($core->IsAbsoluteFilename('fewd.org'                   ));
		$this->CheckFalse($core->IsAbsoluteFilename($protocol . $domain . '/test'));
		$this->CheckFalse($core->IsAbsoluteFilename($protocol . $domain          ));
		$this->CheckTrue( $core->IsAbsoluteFilename($hostRoot . '/test'          ));
		$this->CheckTrue( $core->IsAbsoluteFilename($hostRoot                    ));
		...
		$this->Check($core->AbsoluteFilename('https://fewd.org/test'             ), 'https://fewd.org/test');
		$this->Check($core->AbsoluteFilename('https://fewd.org/test'      , true ), 'https://fewd.org/test');
		$this->Check($core->AbsoluteFilename('https://fewd.org/test'      , false), '');
		$this->Check($core->AbsoluteFilename('ftp://' . $domain . '/test'        ), 'ftp://' . $domain . '/test');
		$this->Check($core->AbsoluteFilename('ftp://' . $domain . '/test' , true ), 'ftp://' . $domain . '/test');
		$this->Check($core->AbsoluteFilename('ftp://' . $domain . '/test' , false), '');
		$this->Check($core->AbsoluteFilename($protocol . $domain . '/test'       ), $hostRoot . '/test');
		$this->Check($core->AbsoluteFilename($protocol . $domain . '/test', true ), $hostRoot . '/test');
		$this->Check($core->AbsoluteFilename($protocol . $domain . '/test', false), $hostRoot . '/test');
		$this->Check($core->AbsoluteFilename($protocol . $domain                 ), $hostRoot);
		$this->Check($core->AbsoluteFilename($protocol . $domain          , true ), $hostRoot);
		$this->Check($core->AbsoluteFilename($protocol . $domain          , false), $hostRoot);
		$this->Check($core->AbsoluteFilename($hostRoot . '/test'                 ), $hostRoot . '/test');
		$this->Check($core->AbsoluteFilename($hostRoot . '/test'          , true ), $hostRoot . '/test');
		$this->Check($core->AbsoluteFilename($hostRoot . '/test'          , false), $hostRoot . '/test');
		$this->Check($core->AbsoluteFilename($hostRoot                           ), $hostRoot);
		$this->Check($core->AbsoluteFilename($hostRoot                    , true ), $hostRoot);
		$this->Check($core->AbsoluteFilename($hostRoot                    , false), $hostRoot);
